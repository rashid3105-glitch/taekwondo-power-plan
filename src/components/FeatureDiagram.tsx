...
function useTooltip() {
  ...
  const show = useCallback((e: React.MouseEvent | React.TouchEvent, text: string) => {
    const svg = (e.currentTarget as Element).closest("svg");
    if (!svg) return;
    const rect = svg.getBoundingClientRect();
    const clientX = "touches" in e ? e.touches[0].clientX : e.clientX;
    const clientY = "touches" in e ? e.touches[0].clientY : e.clientY;
    setTip({ x: clientX - rect.left, y: clientY - rect.top - 8, text });
  }, []);
...
function TipBubble({ tip }: { tip: { x: number; y: number; text: string } | null }) {
  if (!tip) return null;
  return (
    <div
      className="absolute z-50 pointer-events-none px-2.5 py-1.5 rounded-lg bg-popover border border-border shadow-lg text-[10px] sm:text-xs leading-tight text-popover-foreground max-w-[180px] text-center whitespace-pre-line"
...
function PlanDiagram() {
  ...
  const phases = [
    { label: "Adapt", vol: 60, int: 40, color: "hsl(var(--speed))", desc: "Low intensity\nBuild movement base" },
    { label: "Build", vol: 85, int: 60, color: "hsl(var(--energy))", desc: "High volume\nDevelop work capacity" },
    { label: "Peak", vol: 50, int: 90, color: "hsl(var(--explosive))", desc: "Max intensity\nCompetition readiness" },
    { label: "Deload", vol: 30, int: 30, color: "hsl(var(--tab-mental))", desc: "Active recovery\nSuper-compensation" },
  ];
    ...
    <div className="relative">
      <svg viewBox={`0 0 ${W} ${H + 30}`} className="w-full max-w-[280px] sm:max-w-xs mx-auto">
        ...
        {phases.map((p, i) => {
          ...
          return (
            <g key={p.label} className="cursor-pointer" onMouseEnter={(e) => show(e, p.desc)} onMouseLeave={hide} onTouchStart={(e) => show(e, p.desc)} onTouchEnd={hide}>
              ...
              {/* Invisible hit area */}
              <rect x={x - 24} y={H - 2} width="48" height="24" fill="transparent" />
            </g>
          );
        })}
      </svg>
...
function ProgressDiagram() {
  const { tip, show, hide } = useTooltip();
  const bars = [
    { label: "W1", value: 45, color: "hsl(var(--energy))", desc: "Baseline week\n45% training load" },
    { label: "W2", value: 62, color: "hsl(var(--energy))", desc: "Progressive overload\n62% load (+17%)" },
    { label: "W3", value: 78, color: "hsl(var(--energy))", desc: "Building phase\n78% load (+16%)" },
    { label: "W4", value: 55, color: "hsl(var(--speed))", desc: "Deload week\n55% load (recovery)" },
    { label: "W5", value: 85, color: "hsl(var(--energy))", desc: "Peak volume\n85% training load" },
    { label: "W6", value: 92, color: "hsl(var(--explosive))", desc: "Competition prep\n92% peak intensity" },
  ];
    ...
    <div className="relative">
      <svg viewBox={`0 0 ${W} ${H + 20}`} className="w-full max-w-[240px] sm:max-w-[280px] mx-auto">
        ...
        {bars.map((b, i) => {
          ...
          const y = H - PY - barH;
          return (
            <g key={b.label} className="cursor-pointer" onMouseEnter={(e) => show(e, b.desc)} onMouseLeave={hide} onTouchStart={(e) => show(e, b.desc)} onTouchEnd={hide}>
              <defs>
...
function MentalDiagram() {
  const { tip, show, hide } = useTooltip();
  const dims = [
    { label: "Focus", value: 0.8, desc: "Concentration\n80% — strong" },
    { label: "Grit", value: 0.65, desc: "Mental toughness\n65% — developing" },
    { label: "Calm", value: 0.7, desc: "Composure under pressure\n70% — good" },
    { label: "Drive", value: 0.9, desc: "Motivation & intensity\n90% — excellent" },
    { label: "Vision", value: 0.75, desc: "Tactical awareness\n75% — solid" },
  ];
  ...
  return (
    <div className="relative">
      <svg viewBox="0 0 280 210" className="w-full max-w-[200px] sm:max-w-[240px] mx-auto">
        ...
        <motion.polygon points={poly(dims.map(d => d.value))} fill="hsl(var(--tab-mental))" fillOpacity="0.15" stroke="hsl(var(--tab-mental))" strokeWidth="2" initial={{ scale: 0, opacity: 0 }} whileInView={{ scale: 1, opacity: 1 }} viewport={{ once: true }} transition={{ duration: 0.7, ease: "easeOut" }} style={{ transformOrigin: `${cx}px ${cy}px` }} />
        {dims.map((d, i) => {
          const a = -Math.PI / 2 + i * angleStep;
          const lx = cx + Math.cos(a) * (R + 16);
          const ly = cy + Math.sin(a) * (R + 16);
          return <motion.text key={d.label} x={lx} y={ly + 3} textAnchor="middle" fontSize="8" fontWeight="600" className="fill-muted-foreground" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.5 + i * 0.08 }}>{d.label}</motion.text>;
        })}
        {dims.map((d, i) => {
          const a = -Math.PI / 2 + i * angleStep;
          return (
            <motion.circle
              key={i}
              cx={cx + Math.cos(a) * R * d.value}
              cy={cy + Math.sin(a) * R * d.value}
              r="5"
              fill="hsl(var(--background))"
              stroke="hsl(var(--tab-mental))"
              strokeWidth="2"
              ...
              onMouseEnter={(e) => show(e as any, d.desc)}
              onMouseLeave={hide}
              onTouchStart={(e) => show(e as any, d.desc)}
              onTouchEnd={hide}
            />
          );
        })}
...
function RehabDiagram() {
  const { tip, show, hide } = useTooltip();
  const phases = [
    { label: "Protect", pct: 20, color: "hsl(var(--tab-rehab))", desc: "Immobilize & reduce load\nNo kicking / poomse" },
    { label: "Restore", pct: 50, color: "hsl(var(--energy))", desc: "Regain ROM & flexibility\nGentle poomse patterns" },
    { label: "Strengthen", pct: 80, color: "hsl(var(--speed))", desc: "Sport-specific drills\nPoomse stances & turns" },
    { label: "Return", pct: 100, color: "hsl(var(--explosive))", desc: "Full sparring & poomse\nCompetition clearance" },
  ];
  const W = 380, H = 60;
    ...
    <div className="relative">
      <svg viewBox={`0 0 ${W} ${H + 10}`} className="w-full max-w-[260px] sm:max-w-[300px] mx-auto">
        ...
        {phases.map((p, i) => {
          ...
          return (
            <g key={p.label} className="cursor-pointer" onMouseEnter={(e) => show(e, p.desc)} onMouseLeave={hide} onTouchStart={(e) => show(e, p.desc)} onTouchEnd={hide}>
              <motion.circle cx={x} cy={25} r="10" fill={p.color} fillOpacity="0.2" stroke={p.color} strokeWidth="2" initial={{ scale: 0 }} whileInView={{ scale: 1 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.15, type: "spring" }} />
              ...
              {/* Invisible hit area */}
              <rect x={x - 20} y={5} width="40" height="50" fill="transparent" />
            </g>
          );
        })}
...
function NutritionDiagram() {
  const { tip, show, hide } = useTooltip();
  const macros = [
    { label: "Protein", pct: 30, color: "hsl(var(--explosive))", desc: "30% of daily intake\nMuscle repair & growth" },
    { label: "Carbs", pct: 50, color: "hsl(var(--energy))", desc: "50% of daily intake\nPrimary energy source" },
    { label: "Fat", pct: 20, color: "hsl(var(--speed))", desc: "20% of daily intake\nHormone production" },
  ];
  ...
  return (
    <div className="relative">
      <svg viewBox="0 0 200 190" className="w-full max-w-[160px] sm:max-w-[180px] mx-auto">
        {arcs.map((a, i) => (
          <g key={a.label} className="cursor-pointer" onMouseEnter={(e) => show(e, a.desc)} onMouseLeave={hide} onTouchStart={(e) => show(e, a.desc)} onTouchEnd={hide}>
...
function TestingDiagram() {
  const { tip, show, hide } = useTooltip();
  const tests = [
    { label: "Sprint", value: 78, color: "hsl(var(--speed))", desc: "10m sprint time\n78th percentile" },
    { label: "Agility", value: 85, color: "hsl(var(--energy))", desc: "T-test / Illinois\n85th percentile" },
    { label: "Power", value: 70, color: "hsl(var(--explosive))", desc: "Vertical jump\n70th percentile" },
    { label: "Endurance", value: 65, color: "hsl(var(--tab-mental))", desc: "Yo-yo / beep test\n65th percentile" },
  ];
    ...
    <div className="relative">
      <svg viewBox={`0 0 ${W} ${tests.length * gap + 10}`} className="w-full max-w-[240px] sm:max-w-[280px] mx-auto">
        {tests.map((t, i) => {
          ...
          const barW = W - PX - 40;
          return (
            <g key={t.label} className="cursor-pointer" onMouseEnter={(e) => show(e, t.desc)} onMouseLeave={hide} onTouchStart={(e) => show(e, t.desc)} onTouchEnd={hide}>
...
function LibraryDiagram() {
  const { tip, show, hide } = useTooltip();
  const cats = [
    { label: "Strength", icon: "◆", color: "hsl(var(--explosive))", desc: "Squats, deadlifts\nResistance training" },
    { label: "Speed", icon: "⚡", color: "hsl(var(--speed))", desc: "Sprint drills\nReaction training" },
    { label: "Core", icon: "●", color: "hsl(var(--energy))", desc: "Planks, rotations\nStabilization work" },
    { label: "Mobility", icon: "◎", color: "hsl(var(--tab-mental))", desc: "Dynamic stretching\nJoint health" },
    { label: "Power", icon: "▲", color: "hsl(var(--tab-rehab))", desc: "Plyometrics\nExplosive movements" },
    { label: "Balance", icon: "◇", color: "hsl(var(--tab-nutrition))", desc: "Proprioception\nSingle-leg work" },
  ];
    ...
    <div className="relative">
      <svg viewBox={`0 0 ${PX * 2 + cols * cellW + (cols - 1) * gap} ${PX + 2 * cellH + gap + 10}`} className="w-full max-w-[220px] sm:max-w-[260px] mx-auto">
        {cats.map((c, i) => {
          ...
          return (
            <motion.g key={c.label} className="cursor-pointer" initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.08 }} onMouseEnter={(e) => show(e as any, c.desc)} onMouseLeave={hide} onTouchStart={(e) => show(e as any, c.desc)} onTouchEnd={hide}>
