...
export function WeeklySquadExport({ athletes, variant = "icon" }: Props) {
  ...
  const handleExport = async () => {
    ...
    const loadingId = toast.loading(t("weeklyReportGenerating") || "Generating summaries…");
    ...
    try {
      ...
      athleteData.forEach(({ athlete }, idx) => {
        ...
        doc.text("SUMMARY", margin + 10, y + 12);
        doc.setTextColor(0);
        doc.setFont("helvetica", "italic");
        doc.setFontSize(10);
      ...
      doc.text(doc.splitTextToSize(t("assistantDisclosure"), contentWidth), margin, pageHeight - 48);
