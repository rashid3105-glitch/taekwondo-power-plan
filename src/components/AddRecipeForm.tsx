...
import { RECIPE_CATEGORY_LABELS, type RecipeCategory } from "@/data/recipes";
...
export function AddRecipeForm({ onClose, onAdded }: AddRecipeFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!trimmedName || trimmedName.length > 100) {
      toast({ title: "Enter a valid recipe name (max 100 chars)", variant: "destructive" });
      return;
    }
    ...
    if (cal < 0 || cal > 5000 || prot < 0 || prot > 500 || carb < 0 || carb > 500 || f < 0 || f > 500) {
      toast({ title: "Please enter valid macro values", variant: "destructive" });
      return;
    }
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-4 sm:p-5 shadow-card space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-bold text-foreground">Add Custom Recipe</h3>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <div>
            ...
            <Select value={category} onValueChange={(v) => setCategory(v as RecipeCategory)}>
              ...
              <SelectContent>
                {CATEGORIES.map((c) => (
                  <SelectItem key={c} value={c}>{RECIPE_CATEGORY_LABELS[c]}</SelectItem>
                ))}
              </SelectContent>
            </Select>
          </div>
        ...
        <div>
          <Label className="text-xs">Athlete Tip (optional)</Label>
        ...
        <Button type="submit" disabled={saving} className="w-full">
          {saving ? <><Loader2 className="h-4 w-4 mr-1 animate-spin" /> Saving...</> : <><Plus className="h-4 w-4 mr-1" /> Add Recipe</>}
        </Button>
      </form>
    </div>
  );
