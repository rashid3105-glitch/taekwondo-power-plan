    ...
    <>
      ...
      <div className="space-y-2">
        {schedule.map((day: any, i: number) => {
          const sessions = normalizeDaySessions(day);
          const primaryConfig = TYPE_BADGES[sessions[0]?.type] || TYPE_BADGES.gym;
          const PrimaryIcon = primaryConfig.icon;
          const isExpanded = expandedDays.has(i);
          const totalExercises = sessions.reduce((sum: number, s: any) => sum + (s.exercises?.length || 0), 0);
            ...
            <div key={i} className="rounded-lg border border-border bg-secondary/20 overflow-hidden">
              ...
              >
                <div className="flex items-center gap-0.5 flex-shrink-0">
                  {sessions.map((s: PlanSession, si: number) => {
                    const sc = TYPE_BADGES[s.type] || TYPE_BADGES.gym;
                    const SI = sc.icon;
                    return <SI key={si} className="h-4 w-4 text-muted-foreground" />;
                  })}
                </div>
                ...
                <span className="text-sm font-semibold text-foreground flex-1 text-left truncate">
                  {sessions.length > 1 ? sessions.map((s: PlanSession) => s.label).join(" + ") : (sessions[0]?.label || day.label)}
                </span>
                {totalExercises > 0 && (
              ...
              {isExpanded && (
                <div className="px-3 pb-3 pt-1 space-y-3 border-t border-border animate-slide-up">
                  {sessions.map((session: PlanSession, si: number) => {
                    const sessConfig = TYPE_BADGES[session.type] || TYPE_BADGES.gym;
                    const SessIcon = sessConfig.icon;
                    return (
                      <div key={si} className="space-y-2">
                        {sessions.length > 1 && (
                          <div className="flex items-center gap-2 mt-1">
                            <SessIcon className="h-3.5 w-3.5 text-muted-foreground" />
                            <span className="text-xs font-bold text-foreground">{session.label}</span>
                            <span className="text-[10px] text-muted-foreground uppercase">{sessConfig.label}</span>
                          </div>
                        )}
                        {session.focus && (
                          <p className="text-xs text-muted-foreground italic">Focus: {session.focus}</p>
                        )}
