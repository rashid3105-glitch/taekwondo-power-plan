...
interface CoachProfileReadyProps {
  userName?: string
  userEmail?: string
  belt?: string
  discipline?: string
...
const CoachProfileReadyEmail = ({ userName, userEmail, belt, discipline }: CoachProfileReadyProps) => (
  <Html lang="en" dir="ltr">
    <Head />
    <Preview>A user has completed their profile on {SITE_NAME}</Preview>
    <Body style={main}>
      <Container style={container}>
        <Section style={headerSection}>
          <Heading style={h1}>Profile Setup Completed</Heading>
        </Section>
        <Text style={text}>
          A user pending approval has completed their profile on {SITE_NAME}:
        </Text>
        <Section style={detailsBox}>
          ...
          <Hr style={divider} />
          <Text style={detailLabel}>Belt Level</Text>
          <Text style={detailValue}>{belt || 'Not set'}</Text>
          <Hr style={divider} />
          <Text style={detailLabel}>Discipline</Text>
          <Text style={detailValue}>{discipline || 'Not set'}</Text>
        </Section>
        <Text style={text}>
...
export const template = {
  component: CoachProfileReadyEmail,
  subject: (data: Record<string, any>) => `Profile ready for review: ${data.userName || data.userEmail || 'Unknown'}`,
  displayName: 'Profile setup completed notification',
  to: Deno.env.get('ADMIN_NOTIFICATION_EMAIL') || 'rashid3105@gmail.com',
  previewData: { userName: 'Jane Coach', userEmail: 'jane@example.com', belt: 'black', discipline: 'sparring' },
