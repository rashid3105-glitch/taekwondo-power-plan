...
serve(async (req) => {
  ...
  try {
    ...
    // Fixed 18-year product-safety threshold. Fails closed: any lookup problem
    // is treated as "minor", so numeric targets are never generated by accident.
    let isMinor = true;
    try {
      const { data: minorData, error: minorErr } = await supabase.rpc("is_minor", { _user_id: userId });
      isMinor = minorErr ? true : minorData !== false;
    } catch {
      isMinor = true;
    }
