...
const SUBJECTS = [
  (level: number, weakest: string) => `Jeres klubanalyse: niveau ${level}, bremset af ${weakest}`,
  (_level: number, weakest: string) => `${weakest} sætter loftet for jeres talentarbejde`,
  (level: number) => `Niveau ${level} — og de tre skridt i den rigtige rækkefølge`,
]

const DAY_LABELS = ['Dag 1-30', 'Dag 31-60', 'Dag 61-90']
...
function buildHtml(row: Row, unsubUrl: string) {
  ...
  const preheader =
    `${weakestName} sætter loftet for jeres talentarbejde — her er de tre skridt i den rækkefølge, de skal tages.`
    ...
    .map(
      (d, i) => `<tr><td style="padding:0 0 14px;">
        <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%"><tr>
          ...
          <td align="right" style="font:700 13px Arial,Helvetica,sans-serif;color:${MUTED};padding-bottom:6px;">Niveau ${levelOf(scores[i])}</td>
        </tr></table>
        ${bar(i)}
      </td></tr>`,
    )
      ...
      return `<tr><td style="padding:0 0 14px;">
        <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="background-color:${BG};border-radius:10px;">
          <tr><td style="padding:18px 20px;">
            <div style="font:800 11px Arial,Helvetica,sans-serif;letter-spacing:1.2px;color:${GOLD};text-transform:uppercase;">${DAY_LABELS[n]}</div>
            ...
            <div style="font:400 15px/1.65 Arial,Helvetica,sans-serif;color:${INK};"><strong>Gør det her først:</strong> ${esc(d.firstStep)}</div>
          </td></tr>
        </table>
      </td></tr>`
    })
...
<title>Jeres klubanalyse</title></head>
...
<table role="presentation" cellpadding="0" cellspacing="0" border="0" width="600" style="width:600px;max-width:100%;">
  ...
  <tr><td style="background-color:${INK};border-radius:12px 12px 0 0;padding:20px 28px;">
    <div style="font:800 12px Arial,Helvetica,sans-serif;letter-spacing:2px;color:${GOLD};text-transform:uppercase;">Sportstalent · Klubanalysen</div>
  </td></tr>
  ...
  <tr><td style="background-color:${CARD};padding:30px 28px 8px;">
    ...
    <!-- 3. Overskrift -->
    <div style="font:800 22px/1.3 Arial,Helvetica,sans-serif;color:${INK};padding:26px 0 14px;">${esc(clubName)} er bremset af ${esc(weakestName)}</div>

    <!-- 4. Forklaring -->
    <div style="font:400 15px/1.7 Arial,Helvetica,sans-serif;color:${MUTED};padding-bottom:14px;">
      ${esc(roleName)} har gennemført Klubanalysen: 15 spørgsmål om rød tråd, trænerkapacitet, data, kultur og ledelse.
      Niveauet sættes af det svageste af de fem områder — ikke af gennemsnittet. En klub er ikke halvvejs, fordi den er
      stærk ét sted og fraværende et andet. Den er sårbar.
    </div>
    <div style="font:400 15px/1.7 Arial,Helvetica,sans-serif;color:${INK};padding-bottom:24px;">${esc(lvl.verdict)}</div>

    <!-- 5. Fordeling -->
    <div style="font:800 12px Arial,Helvetica,sans-serif;letter-spacing:1.4px;color:${MUTED};text-transform:uppercase;padding-bottom:14px;">Fordeling</div>
    <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%">${distributionRows}</table>

    <!-- 6. De tre skridt -->
    <div style="font:800 22px/1.3 Arial,Helvetica,sans-serif;color:${INK};padding:18px 0 12px;">De tre skridt — i den rækkefølge, de skal tages</div>
    <div style="font:400 15px/1.7 Arial,Helvetica,sans-serif;color:${MUTED};padding-bottom:18px;">
      Rækkefølgen er ikke ligegyldig. Tager I det næststørste hul først, bygger I oven på det største — og så holder
      arbejdet kun, indtil den næste udskiftning.
    </div>
    ...
    <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="background-color:${INK};border-radius:10px;margin:14px 0 8px;">
      <tr><td style="padding:22px 24px;">
        <div style="font:800 11px Arial,Helvetica,sans-serif;letter-spacing:1.6px;color:${GOLD};text-transform:uppercase;">Tag med til næste bestyrelsesmøde</div>
    ...
    <div style="font:400 15px/1.7 Arial,Helvetica,sans-serif;color:${MUTED};padding:22px 0 18px;">
      De tre skridt kræver ingen software. De kræver, at nogen har tid — og at det, der bliver skrevet, stadig findes
      om to år. Det er dér, de fleste klubber løber tør.
    </div>
    <table role="presentation" cellpadding="0" cellspacing="0" border="0"><tr>
      <td style="background-color:${GOLD};border-radius:8px;">
        <a href="${SITE}/contact" style="display:inline-block;padding:14px 26px;font:800 15px Arial,Helvetica,sans-serif;color:${INK};text-decoration:none;">Book en gennemgang af jeres tre huller</a>
      </td>
    </tr></table>
  ...
  <tr><td style="background-color:${CARD};border-radius:0 0 12px 12px;border-top:1px solid ${TRACK};padding:20px 28px 26px;">
    <div style="font:400 12px/1.6 Arial,Helvetica,sans-serif;color:#8A8B8F;">
      Du modtager denne mail, fordi der blev gennemført en klubanalyse på sportstalent.dk den ${esc(fmtDate(row.created_at))} med denne adresse.<br>
      ${esc(COMPANY)}<br>
      <a href="${unsubUrl}" style="color:#8A8B8F;">Afmeld</a> ·
      <a href="${SITE}/privacy" style="color:#8A8B8F;">Privatlivspolitik</a>
    </div>
  </td></tr>
...
function buildText(row: Row, unsubUrl: string) {
  const scores = row.scores || [0, 0, 0, 0, 0]
  ...
  const steps = lowestThree
    .map(({ i }, n) => {
      const d = dims[i]
      return `${DAY_LABELS[n]} — ${d.name}\n${d.consequence}\nGør det her først: ${d.firstStep}`
    })
    .join('\n\n')

  return `SPORTSTALENT · KLUBANALYSEN

Niveau ${level} — ${lvl.name} (${lvl.subtitle})

${clubName} er bremset af ${dims[weakestIdx]?.name ?? row.weakest}

${roleName} har gennemført Klubanalysen: 15 spørgsmål om rød tråd, trænerkapacitet, data, kultur og ledelse. Niveauet sættes af det svageste af de fem områder — ikke af gennemsnittet. En klub er ikke halvvejs, fordi den er stærk ét sted og fraværende et andet. Den er sårbar.
...
DE TRE SKRIDT — I DEN RÆKKEFØLGE, DE SKAL TAGES
...
TAG MED TIL NÆSTE BESTYRELSESMØDE
...
Book en gennemgang af jeres tre huller: ${SITE}/contact
30 minutter, ingen forpligtelser. Vi gennemgår jeres tre huller — også hvis I ender med at løse dem selv.

---
Du modtager denne mail, fordi der blev gennemført en klubanalyse på sportstalent.dk den ${fmtDate(row.created_at)} med denne adresse.
${COMPANY}
Afmeld: ${unsubUrl}
Privatlivspolitik: ${SITE}/privacy`
}
...
Deno.serve(async (req) => {
  ...
  const { data: row, error } = await admin
    ...
    .select('id,email,level,scores,weakest,strongest,club_name,role,created_at,report_sent_at,unsubscribed_at')
    .eq('id', id)
    .maybeSingle()
  ...
  const variantIdx = Math.floor(Math.random() * SUBJECTS.length)
  const weakestName =
    DIMENSION_CONTENT[(row.scores as number[]).indexOf(Math.min(...(row.scores as number[])))]?.name ||
    row.weakest
  const subject = SUBJECTS[variantIdx](row.level, weakestName)
  const variantLabel = String.fromCharCode(65 + variantIdx) // A / B / C

  const html = buildHtml(row as Row, unsubUrl)
  const text = buildText(row as Row, unsubUrl)
