CREATE OR REPLACE FUNCTION public.get_squad_overview(_coach_id uuid)
RETURNS jsonb
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $func$
  WITH guard AS (SELECT (auth.uid() IS NOT NULL AND auth.uid() = _coach_id AND has_role(_coach_id, 'coach'::app_role)) AS ok),
  athletes AS (
    SELECT ca.athlete_id AS user_id FROM public.coach_athletes ca, guard WHERE guard.ok AND ca.coach_id = _coach_id
    UNION
    SELECT p2.user_id FROM public.profiles p1 JOIN public.profiles p2 ON p2.club_id = p1.club_id, guard
    WHERE guard.ok AND p1.user_id = _coach_id AND p1.club_id IS NOT NULL AND p2.user_id <> _coach_id
  ),
  latest_readiness AS (
...
AS $func$
  WITH guard AS (
    SELECT (
      EXISTS (SELECT 1 FROM public.coach_athletes WHERE coach_id = auth.uid() AND athlete_id = _athlete_id)
