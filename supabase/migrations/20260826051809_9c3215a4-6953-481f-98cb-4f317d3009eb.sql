-- 1. Backfill club_id from a single active club membership where possible
UPDATE public.form_curve_weekly f
SET club_id = m.club_id
FROM (
  SELECT user_id, MIN(club_id::text)::uuid AS club_id
  FROM public.club_memberships
  WHERE status = 'active'
  GROUP BY user_id
  HAVING COUNT(DISTINCT club_id) = 1
) m
WHERE f.user_id = m.user_id AND f.club_id IS NULL;

-- 2. Delete rows that cannot be attributed to a club (they regenerate on next compute)
DELETE FROM public.form_curve_weekly WHERE club_id IS NULL;

-- 3. Remove the fallback branch from the coach read policy
DROP POLICY IF EXISTS "Coaches read club form_curve_weekly v2" ON public.form_curve_weekly;
CREATE POLICY "Coaches read club form_curve_weekly v2"
ON public.form_curve_weekly
FOR SELECT
TO authenticated
USING (club_id IS NOT NULL AND public.is_coach_of_club(club_id));

-- 4. Ensure the compute function always resolves a club
...
BEGIN
  ...
  -- Without a club the row would be unreadable/unscoped: skip computation.
  IF v_club_id IS NULL THEN
    RETURN;
  END IF;
