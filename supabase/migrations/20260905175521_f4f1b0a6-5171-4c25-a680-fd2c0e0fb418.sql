...
BEGIN
  ...
  SELECT COALESCE(license_active, false) INTO v_licensed FROM public.clubs WHERE id = v_club;
  ...
  SET is_approved = true,
      ...
      payment_status = CASE
        WHEN v_licensed AND COALESCE(payment_status, '') <> 'paid' THEN 'paid'
        ELSE payment_status END,
      payment_date = CASE
        WHEN v_licensed AND COALESCE(payment_status, '') <> 'paid' THEN current_date
  ...
  RETURN jsonb_build_object('ok', true, 'club_id', v_club, 'coach_id', v_invite.coach_id, 'auto_approved', true, 'paid', v_licensed);
END;
$function$;